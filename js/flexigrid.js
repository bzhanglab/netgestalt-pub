 ...
 * with patch from http://pixelnix.com/flexigrid-jquery-plugin-extending-to-allow-sorting-of-static-grids/
 *
 */
(function ($) {
	/*
			...
			inPlaceSort: function() {
			  ...
			  // then sorting based on that?  TODO: yeah.
				rows.sort(function(a, b) {
					var compA, compB;
					compA = Number($(a).find("td:eq("+col+")").text());
					compB = Number($(b).find("td:eq("+col+")").text());
					if(isNaN(compA) || isNaN(compB)){
						compA = $(a).find("td:eq("+col+")").text();
						compB = $(b).find("td:eq("+col+")").text();
					}
