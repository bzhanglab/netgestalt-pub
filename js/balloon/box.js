...
Box.prototype.addCloseButton = function () {
  ...
  if (!closeButton) {
    ...
    closeButton.onclick = function() {
      Balloon.prototype.nukeTooltip();
			// reenable track label div hover event handler so the icons can be hidden/shown
			/*
			$(".track_delete").hide();
			$(".track-label").hover(function(event){
         ...
         $(this).find(".track_delete").hide();
			...
			*/
			/*
      var tracksExportFormSubmitButton=$('#trackExportFormSubmitButton');
      if($(trackExportFormSubmitButton).length>0){
        $('#trackExportFormSubmitButton').css({'display':'none'});
      }    
      var relatedTracksFormSubmitButton=$('#relatedTracksFormSubmitButton');
      if($(relatedTracksFormSubmitButton).length>0){
        $('#relatedTracksFormSubmitButton').css({'display':'none'});
      }    
      var dtSubmitButton=$('#dtSubmitButton');
      if($(dtSubmitButton).length>0){
        $('#dtSubmitButton').css({'display':'none'});
      }    
      var SCTFilterButton=$('#SCTFilterSubmitButton');
      if($(SCTFilterButton).length>0){
