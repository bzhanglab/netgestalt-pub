function Track(isUserTrack, isUploadTrack, name, key, datatype, loaded, changeCallback, browser) {
    this.name = name;
    ...
    this.isUploadTrack=isUploadTrack;
}
...
Track.prototype.load = function(url) {
    var curTrack = this;
      ...
      type: "GET",
			...
			async: false,
			dataType: "text",
			success: function(o) { curTrack.loadSuccess(o); },
			error: function() {curTrack.loadFail(); } 
		});
};

Track.prototype.loadFail = function() {
    this.empty = true;
    this.setLoaded();
};
...
Track.prototype.showRange = function(first, last, startBase, bpPerBlock, scale,
                                     containerStart, containerEnd, sortPos, commonSampleIndices,
                                     sortedCommonSamples) {
    ...
    if(debug6){
      ...
      console.log("----> firstAttached="+firstAttached+", lastAttached="+lastAttached);
    }
    ...
    for (i = lastAttached; i >= first; i--) {
        ...
        this._showBlock(i, leftBase, leftBase + bpPerBlock, scale,
                        containerStart, containerEnd, first, last, startBase, sortPos,
                        commonSampleIndices, sortedCommonSamples); 
    }
    //fill right
    ...
    if(debug){
      console.log("%%%%% firstAttached="+this.firstAttached+" lastAttached="+this.lastAttached);
    }
    this._adjustBlanks();
...
Track.prototype._showBlock = function(blockIndex, startBase, endBase, scale,
                                      containerStart, containerEnd, first, last, firstStartBase, sortPos,
                                      commonSampleIndices, sortedCommonSamples) {
    // if dragging, don't need to replace the blocks if they already exist
		if(this.blocks[blockIndex] && this.view && this.view.dragging){
    ...
    // redraw the block
      ...
      this.blocks[blockIndex]=undefined;
		...
		/*
    if (this.blocks[blockIndex] && ((this.view && (!this.view.dragging)) || !this.view)) {
      // remove the block
      this.div.removeChild(this.blocks[blockIndex]);
      this.blocks[blockIndex]=undefined;
    }
		*/
		...
		//old version: remove all above 
	  /* 
      if(sortPos===undefined && this.sortPos!==undefined)
       sortPos=this.sortPos;
    
    if (this.blocks[blockIndex] && sortPos===undefined) {
        this.heightUpdate(this.blockHeights[blockIndex], blockIndex);
        return;
    }   
